    ...
    <div class="fnp-page" id="noteEditPage">
        ...
        <div class="fnp-topbar">
            ...
            <div class="fnp-topbar-center">
                ...
                <span class="fnp-breadcrumb-title"
                    id="fnpBreadcrumbTitle">{{ $note?->title ?: ($note ? 'Ghi chú không có tiêu đề' : 'Ghi chú mới') }}</span>
            </div>
                        ...
                        <div class="dropdown">
                            <button type="button" class="btn btn-sm d-flex align-items-center gap-1"
                                ...
                                <span class="material-symbols-outlined"
                                    style="font-size: 18px;">{{ $note->is_locked ? 'lock' : 'lock_open' }}</span>
                            </button>
        ...
        <div class="fnp-editor-wrap">
            ...
            <div class="fn-modal-thumb-preview {{ ($note && $note->attachments->count() > 0) ? '' : 'd-none' }}"
                id="modalThumbPreview">
            ...
            @if($isOwner)
                <div class="fn-inline-img-picker d-none" id="modalInlineImgPicker">
                    ...
                    <input type="file" id="modalInlinePickerInput" accept="image/jpeg,image/jpg,image/png,image/gif,image/webp"
                        class="d-none">
                </div>
            @endif
            ...
            <form action="{{ route('notes.store') }}" method="POST" id="createNoteForm">
                ...
                <div class="fn-modal-field fn-title-row fnp-title-row">
                    <input type="text" name="title" id="modalNoteTitle" class="fn-modal-title-input fnp-title-input"
                        placeholder="Tiêu đề ghi chú" value="{{ $note?->title ?? '' }}" @if(!$isOwner && $sharePermission !== 'edit') readonly @endif required />
                </div>
                ...
                <div id="modalLabelsChips" class="d-none">
                    @foreach($labels as $label)
                        <input type="checkbox" name="label_ids[]" id="modal_label_{{ $label->id }}" value="{{ $label->id }}"
                            class="fn-checkbox-input" {{ ($note && $note->labels->contains($label->id)) ? 'checked' : '' }}
                ...
                <div class="fn-modal-field fn-editor-wrapper fnp-editor-wrapper">
                    <div id="modalNoteContent" class="fn-modal-content-input fnp-content-input"
